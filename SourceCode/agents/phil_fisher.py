...
def phil_fisher_agent(state: AgentState):
    ...
    for ticker in tickers:
        ...
        def normalize(analysis_result):
            if analysis_result['max_score'] > 0:
                return (analysis_result['score'] / analysis_result['max_score']) * 10
            return 0
        
        growth_norm = normalize(growth_quality)
        margins_norm = normalize(margins_stability)
        mgmt_norm = normalize(mgmt_efficiency)
        valuation_norm = normalize(fisher_valuation)
        ...
        fisher_analysis[ticker] = fisher_output.model_dump()
    ...
    data_update = {
    "analyst_signals": {
        "phil_fisher_agent": fisher_analysis
    }
}
