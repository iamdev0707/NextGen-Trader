...
def charlie_munger_agent(state: AgentState):
    ...
    for ticker in tickers:
        ...
        moat_norm = (moat_analysis['score'] / moat_analysis['max_score']) * 10 if moat_analysis['max_score'] > 0 else 0
        mgmt_norm = (management_analysis['score'] / management_analysis['max_score']) * 10 if management_analysis['max_score'] > 0 else 0
        pred_norm = (predictability_analysis['score'] / predictability_analysis['max_score']) * 10 if predictability_analysis['max_score'] > 0 else 0
        valu_norm = (valuation_analysis['score'] / valuation_analysis['max_score']) * 10 if valuation_analysis['max_score'] > 0 else 0
    ...
    data_update = {
        "analyst_signals": {
            "charlie_munger_agent": munger_analysis
        }
    }
